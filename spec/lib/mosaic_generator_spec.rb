...
describe Flickr2Mosaic::MosaicGenerator do
  ...
  it 'should be able to initialize the mosaic generator' do
    expect{Flickr2Mosaic::MosaicGenerator.new(:filenames => filenames, output_file: '/tmp/outspec.jpg')}.not_to raise_error
  end

  it 'should make sure that x-size and y-size per picture are correct' do
    mg = Flickr2Mosaic::MosaicGenerator.new(:filenames => filenames, output_file: '/tmp/outspec.jpg')
  ...
  it 'should be able to resize a picture so that it matches the desired field size' do
    ...
    expect{Flickr2Mosaic::MosaicGenerator.new(:filenames => filenames, output_file: '/tmp/outspec.jpg').send(:resize_picture,'spec/fixtures/ruby_tmp.jpg',1,1)}.not_to raise_error
    #FileUtils.rm( 'spec/fixtures/ruby_tmp.jpg' )
  end
  
  it 'should be able to check the given layout for correctness' do
    expect{Flickr2Mosaic::MosaicGenerator.new(:filenames => filenames, output_file: '/tmp/outspec.jpg').check_layout}.not_to raise_error
     layout = Flickr2Mosaic::MosaicGenerator.new(:filenames => filenames, output_file: '/tmp/outspec.jpg').check_layout
     expect(layout == { 
       1  => [2,1] ,
       2  => [1,1] ,
       3  => [1,1] ,
